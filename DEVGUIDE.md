...
3. [前端开发规范](#3-前端开发规范)
4. [后端开发规范](#4-后端开发规范)
5. [API 对接规范](#5-api-对接规范)（含 [5.4 字段治理](#54-字段治理与-api-命名)）
6. [数据库规范](#6-数据库规范)
7. [标准代码模式](#7-标准代码模式)
...
同一业务含义禁止在 JSON 响应中混用多个键名（例如用户头像同时返回 `avatar` 与 `avatarUrl`）。**新增响应字段前**须查阅 `docs/field-governance.md`，确认是否已有同义字段；数据库旧列可保留，Java 实体旧属性须以 JavaDoc `@deprecated` 标明；删除列或删除 API 键前须完成回填迁移、全仓库引用检查，并通过前端构建与后端测试。完整清单与示例见该文档。
