...
- 事实：旧目录 `frontend/src/modules/fellowship/pages/` 已于 2026-05-01 完全删除（15 个废弃页面）；`modules/fellowship/` 下仍保留 `api/`、`components/`、`composables/`、`stores/`、`utils/` 供模块内引用
- 判断：**运行态独立，页面层历史重复已清理**
...
3. ~~清理历史重复资产：冻结并逐步下线 `frontend/src/modules/fellowship/*` 的重复组件引用风险~~ **已完成**（2026-05-01：`modules/fellowship/pages/` 15 个废弃页面已删除）
