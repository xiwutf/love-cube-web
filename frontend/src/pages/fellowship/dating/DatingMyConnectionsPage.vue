<template>
  <div class="dating-connections-page">
    <NavBar title="我的认识的人" left-arrow @click-left="router.back()" />

    <div v-if="loading" class="loading-wrap"><van-loading size="24" /></div>
    <section v-else>
      <p class="hint">记录你在本场活动中认识过的参与者，便于活动结束后回忆编号</p>
      ...
      <div class="list">
        ...
        >
          ...
          <div class="meta">
            ...
            <p class="time">认识于 {{ formatTime(item.createdAt) }}</p>
          </div>
          <van-icon name="arrow" class="arrow" />
        </article>
        <van-empty v-if="!connections.length" description="还没有认识记录，去花名册看看吧" />
      </div>
    </section>
  </div>
</template>
...
import { fetchDatingMyConnections } from '@/api/datingEvent.js'
...
async function load() {
  loading.value = true
  try {
    connections.value = await fetchDatingMyConnections(eventId.value)
  } catch (e) {
