    ...
    <main class="me-content">
      ...
      <section v-show="!loadingPage" class="profile-card">
        <div class="profile-main">
          ...
          <div v-if="showVipCommerce" class="premium-seal" @click="router.push(fellowshipPath('/vip'))">
            <span>LOVE</span>
      ...
      <section v-show="!loadingPage && showVipCommerce" class="vip-banner" @click="router.push(fellowshipPath('/vip'))">
...
const activityItems = computed(() => {
  ...
  if (s.likesReceived > 0) {
    items.push({
      ...
      time: '累计',
      theme: 'yellow',
      to: fellowshipPath('/liked-me'),
      action: { label: '查看名单', to: fellowshipPath('/liked-me') }
    })
  }
  if (s.todayVisitorCount > 0) {
    items.push({
      ...
      time: '今日',
      theme: 'pink',
      to: fellowshipPath('/messages?tab=visitor'),
      action: { label: '查看访客', to: fellowshipPath('/messages?tab=visitor') }
    })
  }
  return items
})
...
const activityHeaderLink = computed(() => {
  const s = fellowshipStats.value
  if (s.likesReceived > 0) {
    return { label: '喜欢我的人', to: fellowshipPath('/liked-me') }
  }
  if (s.todayVisitorCount > 0) {
    return { label: '谁看过我', to: fellowshipPath('/messages?tab=visitor') }
  }
  return null
})
...
.premium-seal span { font-size: 9px; letter-spacing: 1.4px; }
.premium-seal strong { margin-top: 1px; font-size: 11px; letter-spacing: 0.7px; }
...
.heart-glow::before {
  ...
  box-shadow: 14px -11px 0 rgba(255, 118, 178, 0.78), 28px -4px 0 rgba(255, 86, 160, 0.84);
}
...
.heart-glow::after {
  ...
  box-shadow: -14px 12px 0 rgba(255, 171, 207, 0.66), 12px -13px 0 rgba(255, 94, 166, 0.82);
}
...
.cell-title { margin: 0; font-size: 16px; color: #2a2d42; font-weight: 800; }
...
.cell-sub { margin: 9px 0 0; font-size: 11px; color: #8a8ea3; }
...
.metric-trend { background: linear-gradient(135deg, #d84bff 0%, #7d5cff 100%); }
.metric-fire { background: linear-gradient(135deg, #ffb12e 0%, #ff7a1a 100%); }
.metric-eye { background: linear-gradient(135deg, #6ab7ff 0%, #3989ff 100%); }

.status-title { margin: 0; font-size: 11px; color: #8b90a8; margin-top: 6px; }
.status-value { margin: 2px 0 0; font-size: 22px; font-weight: 700; color: #2a2d42; }
...
.menu-icon :deep(.van-icon) { position: relative; z-index: 1; }
...
.menu-title { margin: 0; font-size: 14px; color: #2c3048; font-weight: 600; }
.menu-sub { margin: 0; font-size: 11px; color: #9ba0b6; }
...
.block-header h3 { margin: 0; font-size: 23px; line-height: 1; color: #23253b; }
.block-link { border: none; background: none; font-size: 13px; color: #9ca1b7; }
...
.remove-photo-btn:disabled { opacity: 0.6; }
...
.upload-item:disabled { opacity: 0.55; }
.upload-item span { font-size: 12px; }
.upload-item small { font-size: 10px; }

.activity-list { margin: 10px 0 0; padding: 0; list-style: none; }
...
.activity-text p { margin: 0; font-size: 14px; color: #2f334a; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.activity-text span { font-size: 12px; color: #9aa0b7; }

.activity-action { border: none; background: none; color: #f14f80; font-size: 13px; }
...
.vip-banner-main { position: relative; z-index: 1; min-width: 0; }
...
.vip-title { margin: 0; font-size: 19px; line-height: 1.08; font-weight: 700; color: #fff7dd; }
.vip-sub { display: block; margin-top: 5px; font-size: 11px; color: rgba(255, 241, 208, 0.78); line-height: 1.35; }
