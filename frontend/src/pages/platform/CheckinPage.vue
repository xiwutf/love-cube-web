<template>
  <section class="ck-page">
    <header class="ck-head">
      <button type="button" class="ck-back" aria-label="返回" @click="$router.back()">‹</button>
...
const dailyLoginTask = computed(() => {
  ...
  // 不强依赖 code 名，按 actionType=LOGIN 识别（后端 GrowthService 会用 actionType 更新进度）
...
async function handleCheckin() {
  ...
  try {
    ...
    // 重复签到会被后端识别为 duplicate，这里统一提示为“已签到”
    await refresh().catch(() => null)
    showToast({ type: 'success', message: isCheckedInToday.value ? '今日已签到' : (error?.message || '签到失败') })
  } finally {
    loading.value = false
  }
}
...
<style scoped>
.ck-page { min-height: 100vh; background: var(--lc-bg); color: var(--lc-text); }
...
.ck-title { flex: 1; margin: 0; font-size: 17px; font-weight: 800; }
...
.ck-muted { margin: 0; color: var(--lc-subtle); font-size: 13px; }

.ck-growth { display: grid; gap: 10px; }
.ck-growth-row { display: flex; justify-content: space-between; align-items: center; gap: 12px; }
.ck-growth-row span { color: var(--lc-subtle); font-size: 12px; font-weight: 700; }
.ck-growth-row strong { color: var(--lc-text); font-size: 13px; font-weight: 900; }
...
.ck-task { display: grid; gap: 10px; }
.ck-task-main { display: flex; justify-content: space-between; align-items: flex-start; gap: 10px; }
.ck-task-name { font-size: 14px; font-weight: 800; color: var(--lc-text); }
.ck-task-sub { margin-top: 3px; font-size: 12px; color: var(--lc-subtle); font-weight: 700; }
.ck-task-sep { margin: 0 6px; opacity: 0.5; }
.ck-pill {
  flex: 0 0 auto;
  border-radius: 999px;
  padding: 4px 10px;
