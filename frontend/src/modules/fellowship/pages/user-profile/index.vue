  ...
  <div class="user-profile-page">
    ...
    <template v-else-if="user">
      ...
      <div class="actions">
        ...
        <van-button round type="primary" icon="friends-o" :loading="requesting" @click="handleMeetRequest">
          申请认识
        </van-button>
      </div>
    </template>
...
import { toFullUrl } from '@f/utils/image.js'
import { storage } from '@f/utils/storage.js'
import request from '@f/api/request.js'
import { likeUser, sendMeetRequest } from '@f/api/match.js'
...
async function handleMeetRequest() {
  ...
  if (!myId) { router.push('/fellowship/login'); return }

  requesting.value = true
  try {
    await sendMeetRequest(route.params.id, `你好，我想认识你！`)
    showDialog({ title: '申请已发送', message: '你的认识申请已发送，等待对方同意～', confirmButtonText: '好的' })
  } catch {
    showToast({ message: '发送失败，请稍后重试', type: 'fail' })
  } finally {
